...
import { filterPublicationsByTitle, filterPublicationsByLocation, filterPublicationsByPrice } from '../store/publicationSlice';
	...
	return (
		<form className="w-96 flex">
			{selectedSearch === 'titulo' ? (
				<input type="text" placeholder="Buscar alojamiento.." className="p-2 border-none rounded-l-full w-full" value={titleFilter} onChange={handleTitleFilterChange} />
			) : (
				<>
					<input
						type="number"
					...
					<input type="number" placeholder="Precio 2" className="ml-3 p-2 border-none w-1/4 mr-2" value={price2Filter} onChange={handlePrice2FilterChange} />
				</>
			)}
			<button type="button" className="p-2 bg-red-500 text-white rounded-full mr-2" onClick={applyTitleFilter}>
				Buscar
			</button>
			<button type="button" className="p-1 bg-red-500 text-white rounded-full" onClick={cancelTitleFilter}>
				Cancelar
			</button>
		</form>
	);
